...
| 为什么长 prompt 不一定一次塞进 GPU？ | token budget、四本资源账与 chunked prefill | [Chunked Prefill 源码状态机](./internals/chunked-prefill) |
...
先看[学习地图与版本边界](./guide/learning-path)选择路线；若目标是从源码反推系统，而不只是会启动服务，直接执行[六周源码学习计划](./guide/study-plan)。

## 深读入口：每页解决一个可验证问题

| 入口 | 读完必须能证明 |
| --- | --- |
| [进程与消息逐跳追踪](./internals/message-flow) | 一个 `rid` 跨 HTTP、ZMQ、Scheduler、模型和 detokenizer 时，消息类型与所有权怎样变化 |
| [Chunked Prefill 源码状态机](./internals/chunked-prefill) | `PrefillAdder` 怎样同时核算 input、chunk、KV 与 request row，最后一个 chunk 怎样转入 decode |
| [PD 解耦与 HiCache](./advanced/pd-hicache) | bootstrap、目标端预分配、KV transfer、`prebuilt` 接管以及 L1/L2/L3 restore 的条件和失败清理 |
| [RL rollout 生命周期](./advanced/rl-lifecycle) | pause/retract/in-place、权重更新、cache 失效、验权与 resume 怎样组成一次安全切换 |
| [八组可验收实验](./practice/lab-workbook) | 每个结论都有命令、预期日志/指标、失败分支和通过标准 |
...
现在先读[六周源码学习计划](./guide/study-plan)，建立证据目录；第一条源码 trace 从[进程与消息逐跳追踪](./internals/message-flow)开始。
