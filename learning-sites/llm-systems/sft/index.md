...
本站主线固定阅读 TRL [`f3adc504`](https://github.com/huggingface/trl/tree/f3adc504b93d634666c5628e7bdaa99ec8861028) 与 Transformers [`e52d0fd6`](https://github.com/huggingface/transformers/tree/e52d0fd6fa9eb874f7c2da048198276b04c919b9)，LoRA/分布式部分再固定 PEFT [`cea82131`](https://github.com/huggingface/peft/tree/cea8213158c8b682acc0839405c2062d57fdf867) 与 Accelerate [`665444ce`](https://github.com/huggingface/accelerate/tree/665444ceb62211f2b410d0d0fdb4bc013c5effdf)。配置默认值以源码与实际 resolved config 为准，不把旧教程中的行为套到当前版本。
...
| 目标 | 顺序 | 最终验收 |
| --- | --- | --- |
| 第一次完成 SFT | 学习计划 → objective/data/template/mask → 第一次训练 → evaluation | tiny-overfit、held-out generation、可加载 checkpoint |
| 读懂并维护训练栈 | 基础四课 → architecture/data-pipeline → [源码反推主实验](./internals/source-walkthrough) → loss-update | 从 raw sample 追到 optimizer delta，并跑反例矩阵 |
| 做 LoRA/多卡生产实验 | 基础与 first-run → LoRA/QLoRA → scaling → debugging | 全参/LoRA/QLoRA 对照、单卡/分布式对照、恢复演练 |

不要从多卡命令开始。单卡 token audit 与 one-step update 没通过时，多卡只会把错误复制或分片。
...
完整源码行号、启动条件、张量例子、断点与故障注入集中在[从构造器到一次参数更新](./internals/source-walkthrough)。
