...
本站不是只讲概念的速览。建议先下载同一份源码，再按“计划 → 完整调用链 → GPU 数据路径 → 运行时证据 → 分布式验收”推进：
...
git checkout --detach 61141ed265bfef41a0ca19e992567ea980919b96
git rev-parse HEAD
```

| 课程 | 最终产物 |
| --- | --- |
| [30 日六阶段源码学习计划](./guide/study-plan) | 每日阅读、实验、证据与验收清单 |
| [固定提交完整调用链](./internals/full-code-path) | HTTP 到 SSE 的函数、对象、进程与状态变化表 |
| [模型 forward、Paged KV 与采样](./internals/model-forward-sampling) | SchedulerOutput 到 Llama attention、logits 和 token 的数据路径 |
| [源码跟踪实验](./practice/source-lab) | 固定版本、CPU 单测、请求 trace、指标与 profile 报告 |
| [分布式实战](./practice/distributed-lab) | 单卡、TP/mp、TP/Ray、PP、DP 与多节点逐级验收 |

概念页负责推导“为什么”，上述深读页负责证明“这份源码实际上怎样做”。不要在没有完成上一层验收时直接跳到多节点。
