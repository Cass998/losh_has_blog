...
```python
scheduler_output = scheduler.schedule(should_throttle_prefills)
...
process_aborts_queue()
engine_core_outputs = scheduler.update_from_output(
    scheduler_output, model_output
)
```

短不等于简单。复杂性被压进两个稳定契约：Scheduler 负责“这一轮做谁、做多少、用哪些 block”，Runner 负责“按计划构造 GPU batch 并执行”。这里的条件分支不能省：固定提交启用 **V2 ModelRunner 的生成路径**时，`execute_model()` 完成 forward、把 hidden states 存入 `execute_model_state`，末 PP stage 返回 `None`；Core 因而调用 `sample_tokens(grammar_output)`。若配置回退到 **V1 ModelRunner**，`execute_model()` 已经返回 `ModelRunnerOutput`，Core 就跳过第二次采样。实际选择由 `VllmConfig.use_v2_model_runner` 和启动日志决定，而不是由“Engine V1”这个总名称决定。
...
sequenceDiagram
    ...
    S->>E: SchedulerOutput
    E->>R: execute_model non_block=True
    E-->>S: future
    S->>S: get_grammar_bitmask
    R->>R: update persistent batch + prepare tensors
    R->>R: model forward + save hidden states
    R-->>E: V2 generate returns None
    E-->>S: future.result gives None
    S->>E: sample_tokens grammar_output
    E->>R: consume execute_model_state + sample
    R-->>E: ModelRunnerOutput
    E-->>S: sampled output
    S->>S: update token counts / finish / free or preempt
```

上图固定画的是 V2 generate 主线。grammar bitmask 在非阻塞 forward 发出后、等待 `future.result()` 前由 Scheduler 准备，因此 CPU 控制工作可与设备执行重叠；`sample_tokens` 仍必须等 future 完成。V1 fallback 把 `model forward + sample` 合并在 `execute_model()` 中，并直接返回 `ModelRunnerOutput`；它不会再走图中的 `sample_tokens` 调用。PP 非末 stage 会产生/发送 `IntermediateTensors`，最终仍由末 stage 持有 hidden states、采样并广播 token。
...
Runner 维护一个跨 step 的 persistent batch，而不是每轮从零构造所有请求。V2 generate 主线根据 `SchedulerOutput`：
...
4. `execute_model()` 运行模型 forward 并保存 hidden states；
5. Core 等待 future，同时取得 Scheduler 生成的 grammar bitmask；
6. `sample_tokens()` 消费刚保存的状态，计算 logits，应用 penalty、grammar 与 sampling；
7. 返回 sampled token ids 与附加输出。

V1 fallback 的职责相同，但第 4–6 步仍封装在 Runner 的一次 `execute_model()` 调用里。读 trace 时先确认 Runner 版本，不能只按函数名猜路径。
...
从 `ChatCompletionRequest` 开始，口述到 SSE chunk 的十次数据变形；指出两个跨进程边界；解释 stop token 与 stop string 为什么可能由不同层完成；说明断连如何释放 KV。

接下来先用[固定提交完整调用链](./full-code-path)把每一站的函数、输入输出、状态修改和验证方法钉到源码，再深入 [Scheduler 与 KV Cache](./scheduler-kv)。
